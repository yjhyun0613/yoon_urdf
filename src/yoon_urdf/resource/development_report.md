...
6가지 다양한 파라미터 조합 시나리오 및 4가지 화각(FOV) 시나리오를 구성하여, 각 알고리즘 모드(`basic`, `advanced_filter`, `sliding_window`)의 성능과 복원 품질을 대시보드를 통해 비교 검증하였습니다.

### 실험 시나리오 조합 결과

| 시나리오명 | 알고리즘 모드 | 카메라 화각 (FOV) | ORB 특징점 수 | 기선/회전 기준 | 복소 크기 | 복원 포인트 수 | 맵 신뢰도 및 노이즈 수준 |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **Default** | `advanced_filter` | 90° | 3000 | 0.12m / 0.20rad | 0.05m | **328** | **매우 높음** (잔상 및 공중 노이즈 없음) |
| **Dense** | `advanced_filter` | 90° | 5000 | 0.06m / 0.10rad | 0.03m | **1529**| **매우 높음** (조밀하고 선명한 원기둥 형상 복원) |
| **Strict_Filter** | `advanced_filter` | 90° | 3000 | 0.20m / 0.26rad | 0.05m | **23**  | **높음** (매우 보수적인 필터링 적용) |
| **Lenient_Filter**| `basic` | 90° | 3000 | 0.05m / 0.08rad | 0.05m | **1558**| **낮음** (극심한 픽셀 오매칭으로 인한 공중 노이즈 유입) |
| **High_Resolution**| `sliding_window` | 90° | 3000 | 0.12m / 0.20rad | 0.02m | **57**  | **높음** (3뷰 기하제약으로 인해 가장 선명하고 왜곡 없는 좌표 복원) |
| **Fast_Speed** | `sliding_window` | 90° | 1000 | 0.15m / 0.20rad | 0.08m | **40**  | **높음** (특징점 수가 작음에도 원기둥 중심선 복원 성공) |
| **FOV_60** | `sliding_window` | 60° (협각) | 3000 | 0.12m / 0.20rad | 0.05m | **14**  | **높음** (좁은 뷰포트로 인해 3뷰 추적 유지 실패로 매우 성김) |
| **FOV_75** | `sliding_window` | 75° | 3000 | 0.12m / 0.20rad | 0.05m | **30**  | **높음** (협각 대비 추적 유지율 향상) |
| **FOV_90** | `sliding_window` | 90° (광각) | 3000 | 0.12m / 0.20rad | 0.05m | **27**  | **높음** (기본 광각 설정) |
| **FOV_105** | `sliding_window` | 105° (초광각) | 3000 | 0.12m / 0.20rad | 0.05m | **69**  | **매우 높음** (넓은 뷰포트로 인해 장시간 3뷰 추적 성공으로 최적 밀도) |

### 화각(FOV) 파라미터 영향 분석

![FOV Comparison Dashboard](./images/fov_comparison.png)

* **협각(FOV 60°)에서의 문제점 (좌측):**
  * 카메라 화각이 좁아질수록 장애물들이 뷰포트 내에 머무르는 시간이 극도로 짧아집니다.
  * 로봇이 회전하고 기동할 때 특징점들이 화면 밖으로 빠르게 빠져나가므로, **3프레임 이상의 슬라이딩 윈도우 체인 매칭이 쉽게 끊어져** 복원되는 포인트 개수가 **14개**로 극히 성글게 나타납니다.
* **초광각(FOV 105°)에서의 이점 (우측):**
  * 화각이 넓어지면 주변 장애물들이 뷰포트 내에 장시간 안정적으로 머무르게 됩니다.
  * 로봇이 주행하는 동안 동일한 특징점들이 카메라 3뷰 체인에 지속적으로 포착되어 **삼각측량 성공률이 비약적으로 증가**하고, 그 결과 가장 조밀하고 깨끗한 **69개**의 3D 포인트가 기둥 중심을 따라 완벽하게 복원됩니다.
